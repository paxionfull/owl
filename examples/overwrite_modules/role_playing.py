...
class RolePlaying:
    ...
    def __init__(
        ...
        *,
        ...
        task_prompt: str = "",
        user_profile: str = "",
        with_prompt_augmentation: bool = False,
        ...
        prompt_augmentation_agent_kwargs: Optional[Dict] = None,
        ...
        self.with_prompt_augmentation = with_prompt_augmentation
        ...
        self.augmented_prompt: Optional[TextPrompt] = None
        self._init_augmented_prompt(
            assistant_role_name,
            user_role_name,
    ...
    def _init_augmented_prompt(
        ...
        """
        1. 添加个人知识库
        2. 根据不同任务类型的guideline，对prompt进行调整
        3. 语言
        """
...
## 要求:
1. 用户画像信息profile中若提供了与该任务相关的信息, 要利用这些信息
2. 不要凭空添加task_prompt和user_profile中没有的信息
...
- 如果task_prompt中不包含profile_id, 请在优化后加入profile_id.
...
- 如果task_prompt中包含了生成html. 优化后的prompt需要加入html的默认样式，例如按照相册的风格生成html.
...
# - 最后的文案默认写入文件outputs/tmp.md或者outputs/tmp.txt.

        if self.with_prompt_augmentation:
            prompt_augment_agent = ChatAgent(
                system_message="",
                output_language=output_language,
                **(prompt_augmentation_agent_kwargs or {}),
            )
            self.augmented_prompt = prompt_augment_agent.step(
            ...
            self.task_prompt = TextPrompt(self.augmented_prompt)
