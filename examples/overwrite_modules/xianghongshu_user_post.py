...
def save_cookies(context, cookie_file='xiaohongshu_cookies.json'):
    """保存浏览器Cookie到文件"""
...
def load_cookies(context, cookie_file='xiaohongshu_cookies.json'):
    """从文件加载Cookie到浏览器"""
...
def manual_login(page, timeout=20):
    """等待用户手动登录并保存Cookie"""
...
def extract_xiaohongshu_notes(profile_id, cookie_file='xiaohongshu_cookies.json', force_login=False):
    """提取小红书用户的笔记内容"""
    ...
    with sync_playwright() as p:
        ...
        login_success = False
        if not force_login:
            if load_cookies(context, cookie_file):
                # 访问首页检查登录状态
                page.goto("https://www.xiaohongshu.com/")
                time.sleep(3)
        ...
        if not login_success:
            ...
            login_success = manual_login(page)
            if login_success:
                # 登录成功后保存Cookie
        ...
        # 提取前5个笔记内容（可以根据需要调整数量）
        max_notes = min(5, len(note_links))
        ...
        for i in range(max_notes):
            try:
                ...
                print(f"提取到的内容: {content[:50]}...")  # 只打印前50个字符
...
if __name__ == "__main__":
    ...
    # profile_id = "599f8e8450c4b42cf42b3012"

    # 设置force_login=True可以强制重新登录
    extract_xiaohongshu_notes(profile_id, force_login=False)
