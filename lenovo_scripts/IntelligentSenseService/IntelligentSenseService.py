        ...
        if power_setting == guid_bytes(GUID_LIDSWITCH_STATE_CHANGE):
            ...
            if lid_state:#open
                if _first_lid_open:
                    ...
                    _first_lid_open = False
                else:
                    logging.info("🔓 执行盖子打开逻辑...")
                    response = send_message_to_pipe(
                                    message=0xFD  # 转换为单字节0xFD
                                    )
                    logging.info(f"单字节响应: {response.hex().upper()}")
                    #打开task list 完成
                    if os.path.exists(result_file):
                        logging.info(f"检测到结果文件，打开: {result_file}")
                        open_html_file_in_browser(result_file)
                    else:
                        logging.info(f"未检测到结果文件，打开默认文件: {default_file}")
                        if os.path.exists(default_file):
                            open_html_file_in_browser(default_file)
                        else:
                            logging.warning(f"默认文件也不存在: {default_file}")
            else:#close
                logging.info("🔒 执行盖子合上逻辑...")
                response = send_message_to_pipe(
                                message=0xFE  # 转换为单字节0xFE
                                )
                logging.info(f"单字节响应: {response.hex().upper()}")
                
                # 检查 result.md 是否已存在
                if os.path.exists(result_file):
                    logging.info(f"检测到结果文件已存在: {result_file}，跳过执行Python脚本")
                    # 直接进入文件检测循环，等待文件被处理
                    check_file_repeatedly(result_file)
                else:
                    ...
                    # # 使用系统默认方式打开 result_file 文件，并 sleep 30 秒
                    # try:
                    #     if os.path.exists(result_file):
                    #         os.startfile(result_file)
                    #         logging.info(f"已用系统默认方式打开文件: {result_file}")
                    #         time.sleep(30)
                    # except Exception as e:
                    #     logging.error(f"打开文件或 sleep 时出错: {e}")
